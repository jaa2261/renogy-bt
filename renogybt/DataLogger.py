...
class DataLogger:
    ...
    def log_file(self, json_data):
        filename = self.config["file_logging"]["file"]
        with open(filename, "w") as file:
            file.write(json.dumps(json_data))
        logging.info(f"JSON data written to file {filename}")
    ...
    def log_remote(self, json_data):
        ...
        (
            ...
            else logging.error(f"Log remote error {req.status_code}")
        )
